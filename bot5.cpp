...
Json::Value randomAvailablePosition()
{
 ...
 int temp[15][15];
	...
	MOVE pre = myboard.preMOVE();
		if (pre.x == 14 && pre.y == 14) {
...
//			else
	...
	//	res = myboard.negaMax(6, alpha, beta, myboard.AIColor);
		res = myboard.minMax(alpha,beta,8,myboard.AIColor);
		MOVE DecideMove = res.returnMove;
		x = DecideMove.x; y = DecideMove.y;
			}
...
int main(){
	...
	for (int i = 0; i < turnID; i++) {
		...
		if(whoturns()%2==1)
		{
			...
			placeWHITE(input["requests"][i]["x"].asInt(), input["requests"][i]["y"].asInt());
		}

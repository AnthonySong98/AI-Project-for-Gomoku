...
void Gomoku::StartAIGame(int color)
{
	int x; int y;
	Print();
...
vector<vector<int>> Gomoku::GenerateNeighbourMove()
{
	...
	for (i = 0; i<15; i++)
		...
		{
			if (Grid[i][j] == 1) {
				if (Grid[i - 1][j - 1] == 0) { oneMove[0] = i - 1; oneMove[1] = j - 1; res.push_back(oneMove); Grid[i - 1][j - 1] = 2; }
				if (Grid[i - 1][j] == 0) { oneMove[0] = i - 1; oneMove[1] = j; res.push_back(oneMove); Grid[i - 1][j] = 2; }
				if (Grid[i - 1][j + 1] == 0) { oneMove[0] = i - 1; oneMove[1] = j + 1; res.push_back(oneMove); Grid[i - 1][j + 1] = 2; }
				if (Grid[i][j - 1] == 0) { oneMove[0] = i; oneMove[1] = j - 1; res.push_back(oneMove); Grid[i][j - 1] = 2; }
				if (Grid[i][j + 1] == 0) { oneMove[0] = i; oneMove[1] = j + 1; res.push_back(oneMove); Grid[i][j + 1] = 2; }
				if (Grid[i + 1][j - 1] == 0) { oneMove[0] = i + 1; oneMove[1] = j - 1; res.push_back(oneMove); Grid[i + 1][j - 1] = 2; }
				if (Grid[i + 1][j] == 0) { oneMove[0] = i + 1; oneMove[1] = j; res.push_back(oneMove); Grid[i + 1][j] = 2; }
				if (Grid[i + 1][j + 1] == 0) { oneMove[0] = i + 1; oneMove[1] = j + 1; res.push_back(oneMove); Grid[i + 1][j + 1] = 2; }
...
double Gomoku::EvaluatePoint(int i, int j)
{
	...
	return  AliveFour(i, j) * 30000000 + DeadFourA(i, j) * 2500 + DeadFourB(i, j) * 3000 + DeadFourC(i, j) * 2600 + AliveThree(i, j) * 3000 + DeadThressA(i, j) * 500 + DeadThressB(i, j) * 800 + DeadThressC(i, j) * 600 + DeadThressD(i, j) * 550 + AliveTwo(i, j) * 650 + DeadTwoA(i, j) * 150 + DeadTwoB(i, j) * 250 + DeadTwoC(i, j) * 200;
	else if (Grid[i][j] = CurrentColor % 2 + 1) {
		double res = 0; CurrentColor = CurrentColor % 2 + 1;
		res=AliveFour(i, j) * 30000000 + DeadFourA(i, j) * 2500 + DeadFourB(i, j) * 3000 + DeadFourC(i, j) * 2600 + AliveThree(i, j) * 3000 + DeadThressA(i, j) * 500 + DeadThressB(i, j) * 800 + DeadThressC(i, j) * 600 + DeadThressD(i, j) * 550 + AliveTwo(i, j) * 650 + DeadTwoA(i, j) * 150 + DeadTwoB(i, j) * 250 + DeadTwoC(i, j) * 200;
